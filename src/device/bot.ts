...
export class Bot {
  ...
  constructor(private readonly platform: SwitchBotPlatform, private accessory: PlatformAccessory, public device: device & devicesConfig) {
    ...
    if (device.bot?.deviceType === "switch") {
      ...
      this.outletService.getCharacteristic(this.platform.Characteristic.On).onGet(async () => {
        //this.infoLog(`${this.device.deviceType}: ${accessory.displayName} onGet Characteristic: ${this.On}`);
        this.infoLog(
          `Goran - his.device.bot?.deviceType = ${this.device.bot?.deviceType}: ${accessory.displayName} onGet Characteristic: ${this.On}`,
        );
  ...
  async openAPIpushChanges(): Promise<void> {
    ...
    this.debugLog(
      `${this.device.deviceType}: ${this.accessory.displayName} - mode: ${this.botMode} openAPIpushChanges` +
